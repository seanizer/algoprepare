    ...
    class Solution implements Balanced {
        ...
        public boolean isBalanced(AbstractBstNode<?, ?> node) {
            ...
            node.inOrderTraversal((n) -> {
                if (!failed.get()) {
                    Integer leftDepth = Optional.ofNullable(n.getLeft()).map(AbstractBstNode::depth).orElse(0);
                    Integer rightDepth = Optional.ofNullable(n.getRight()).map(AbstractBstNode::depth).orElse(0);
                    if (Math.abs(leftDepth - rightDepth) > 1) failed.set(true);
                }
