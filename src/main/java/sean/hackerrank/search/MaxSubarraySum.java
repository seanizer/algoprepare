...
public class MaxSubarraySum {
  ...
  static long maximumSum(long[] a, long m) {
    ...
    Arrays.stream(a)
          .forEach(l -> frequenciesByValue.compute(l % m, (k, v) -> (v == null ? 0 : v) + 1));
  ...
  private static void checkMod(long key, List<Long> context, Map<Long, Integer> frequenciesByValue,
                               AtomicLong max, AtomicBoolean stop, long target) {
    if (stop.get()) {
      return;
    }
    try {
