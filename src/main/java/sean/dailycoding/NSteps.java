...
public interface NSteps {
    ...
    class Solution implements NSteps {
        ...
        public Set<List<Integer>> possibleStepPatterns(int length, Set<Integer> allowedSteps) {
            ...
            gatherStepPatterns(length, Collections.unmodifiableSet(allowedSteps), Collections.emptyList(), 0, result);
            return result;
        }

        private void gatherStepPatterns(int totalLength, Set<Integer> allowedSteps, List<Integer> previousSteps, int currentLength, Set<List<Integer>> result) {
            for (Integer possibleStep : allowedSteps) {
                ...
                if (newLength == totalLength) {
                    ...
                    gatherStepPatterns(totalLength, allowedSteps, append(previousSteps, possibleStep), currentLength + possibleStep, result);
                }
            }
        }
    }
